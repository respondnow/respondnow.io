...
const PostHero = ({ frontmatter }: Pick<BlogPostProps, 'frontmatter'>) => {
  return (
    <BoundedContainer className="pt-14">
      <Link href="/blog">
        <div className="text-xs font-medium text-hint items-center gap-1 inline-flex">
      ...
      <div className="flex items-center justify-between gap-4 mt-4">
        <div className="flex items-center gap-2">
          <Image src={frontmatter.author_image} alt={frontmatter.author} width={40} height={40} />
          <Link href={frontmatter.author_linkedin} target="_blank" rel="noopener noreferrer">
            <p className="font-medium text-sm md:text-base text-[#7D28DE]">{frontmatter.author}</p>
          </Link>
        </div>
        <p className="text-xs md:text-sm text-[#525252]">{frontmatter.date}</p>
      </div>
    </BoundedContainer>
  );
};
